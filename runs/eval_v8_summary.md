...
The canonical eval holdout was re-pinned to `runs/eval_v8_holdout.json` (Phase 1B
...
## Results — `runs/eval_v8_holdout.json` (120 rows, 17 off-topic / 103 on-topic)
...
Raw outputs: `runs/eval_v8_base.txt`, `runs/eval_v8_adapter.txt` (full
mismatches + per-tool argument grounding).

## Re-pin (Phase 1B)

- Created `runs/eval_v8_holdout.json` via `pin_holdout.py --out`.
- Repointed all four live consumers to v8 in one commit: `Makefile`
  ...
  README, SKILL, `runs/caveats.md`.
- **`runs/eval_v5_holdout.json` (the 98-row v6/v7 pin) was left untouched** —
...
as history (see `runs/eval_deepseek_summary.md`, `runs/eval_glm_summary.md` /
